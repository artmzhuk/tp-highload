...
| **Backend**                      | Написан на Go, будут использоваться паттерны graceful shutdown для нормальной остановки микросервисов при обновлениях                                                                    |
| **API Gateway**                  | Будем использовать паттерн circuit bracker, для ограничения запросов на проблемные микросервисы                                                                                          |
...
![Схема](https://github.com/user-attachments/assets/97a7b774-17fe-4cbe-9bbf-81dc5dfa48dd)
...
1. **Поиск (Go, тяжелая логика)**:
    - 10 RPS = 100 МБ RAM
    - 1 RPS = 0.1 ГБ RAM

2. **Остальные сервисы (Go, средняя логика)**:
    - 100 RPS = 100 МБ RAM
    - 1 RPS = 0.001 ГБ RAM

3. **RPS на один датацентр** = RPS / 6 (распределение на 6 датацентров).

---

### Таблица расчетов:

| **Микросервис**      | **RPS на ДЦ** | **Характер**       | **CPU ядра**              | **RAM (ГБ)**               |
|----------------------|---------------|--------------------|---------------------------|----------------------------|
| Поиск                | 2714,33       | Go, тяжелая логика | ( 2714,33 \ 10 = 271,43 ) | ( 2714,33 * 0.1 = 271,43 ) |
| Просмотр жилья       | 1742,50       | Go, средняя логика | ( 1742,5 \ 100 = 17,43 )  | ( 1742,5 * 0.001 = 1,74 )  |
| Бронирование         | 442,33        | Go, средняя логика | ( 442,33 \ 100 = 4,42 )   | ( 442,33 * 0.001 = 0,44 )  |
| Профиль пользователя | 221,17        | Go, средняя логика | ( 221,17 \ 100 = 2,21 )   | ( 221,17 * 0.001 = 0,22 )  |
| Отзывы               | 1809,50       | Go, средняя логика | ( 1809,5 \ 100 = 18,10 )  | ( 1809,5 * 0.001 = 1,81 )  |
| Чаты                 | 452,50        | Go, средняя логика | ( 452,5 \ 100 = 4,53 )    | ( 452,5 * 0.001 = 0,45 )   |
| Рекомендации         | 445,67        | Go, средняя логика | ( 445,67 \ 100 = 4,46 )   | ( 445,67 * 0.001 = 0,45 )  |

---

### Итоговые ресурсы:

| **Ресурс**            | **CPU Ядра** | **RAM (ГБ)** |
|-----------------------|--------------|--------------|
| **Поиск**             | 271.43       | 271.43       |
| **Остальные сервисы** | 51.15        | 5.11         |
| **Итого**             | **322.58**   | **276.54**   |


Для оптимизации количества серверов можно объединить микросервисы с меньшей нагрузкой на одном сервере.

---

| **Микросервисы**                                          | **CPU Cores** | **RAM (ГБ)** | **Серверов** |
|-----------------------------------------------------------|---------------|--------------|--------------|
| Поиск                                                     | 271.43        | 271.43       | 9            |
| Просмотр жилья + Отзывы                                   | 35.53         | 3.55         | 2            |
| Бронирование + Профиль пользователя + Чаты + Рекомендации | 15.62         | 1.56         | 1            |
...
### Таблица с рассчитанными ресурсами

| **Таблица**          | **Размер индекса (ГБ)** | **Кол-во записей** | **Размер БД (ГБ)** | **RPS**  | **CPU ядра** | **RAM (ГБ)** | **СУБД**      |
|----------------------|-------------------------|--------------------|--------------------|----------|--------------|--------------|---------------|
| **User**             | 2.15                    | 537,000,000        | 545                | 6,930    | 14           | 32(индексы)  | Cassandra     |
| **UserAvatar**       | 0.54                    | 134,000,000        | 134,000            | 6,930    | 14           | 32(индексы)  | S3            |
| **Session**          | 1.04                    | 260,000,000        | 77                 | 48,600   | 17           | 128          | Tarantool     |
| **Property**         | 0.11                    | 27,000,000         | 6                  | 2,714.33 | 6            | 32           | Cassandra     |
| **Property_Feature** | 1.62                    | 405,000,000        | 44                 | 2,714.33 | 6            | 64           | Cassandra     |
| **PropertyPhoto**    | 3.24                    | 810,000,000        | 810,000            | 2,714.33 | 6            | 32(индексы)  | S3            |
| **PropertySearch**   | 0.11                    | 27,000,000         | 6                  | 16,286   | 33           | 32           | ElasticSearch |
| **Room**             | 0.11                    | 27,000,000         | 1.3                | 2,714.33 | 6            | 32           | Cassandra     |
| **Booking**          | 5.36                    | 1,340,000,000      | 34                 | 442.33   | 1            | 64           | Cassandra     |
| **Review**           | 1.62                    | 405,000,000        | 200                | 1,809.50 | 4            | 256          | Cassandra     |
| **Favourites**       | 0.05                    | 13,500,000         | 0.15               | 1,742.50 | 4            | 32           | Cassandra     |
| **Chat**             | 5.36                    | 1,340,000,000      | 21                 | 452.50   | 1            | 32           | Postgres      |
| **Message**          | 10.72                   | 2,680,000,000      | 1,300              | 452.50   | 1            | 32(индексы)  | Postgres      |


CPU ядра брались из расчёта 500RPS на ядро, кроме Session -- там 3000 RPS


Модель развертывания: Kubernetes.
Хоститься будет на собственном железе. 
