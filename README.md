...
6. **Среднее количество действий пользователя в день** В среднем пользователь ищет жилье по 3-4 запросам, сохраняет 2-3
   жилья в избранное и делает бронирование. В сумме выходит 7-8 действий в день
...
| Среднее количество действий в день              | 7-8 действий |
...
3. **RPS (requests per second)**: через devtools установлено, что в среднем в секунду от одного пользователя на сервер
   отпралвяется 10 запросов. Значит от всех пользователей поступает 3-5 тысяч запросов в секунду, в пиковое время до 20
   тысяч.
