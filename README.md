...
Денормализуем propertyFeature, добавив к каждому объекту размещения его опции. Оставим отдельую таблицу для ведения общего списка опций.
Денормализуем Review, добавив топ3 отзывов к каждому объекту размещения. Уберем связь с Booking. Добавим в review имя пользователя
Денормализуем Room, добавив цену самой дешевой комнаты к объекту размещения. Будем хранить в Cassandra, в качестве primary key сделаем property_id.
...
| Hbase      | БД сбора данных о рекомендациях  | Быстрый доступ                                                                  |
| Spark      | Обработки данных о рекомендациях |                                                                                 |
